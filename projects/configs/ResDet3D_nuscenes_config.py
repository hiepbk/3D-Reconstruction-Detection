...
# Import custom hooks
custom_imports = dict(
    imports=['projects.mmdet3d_plugin.core.hook.val_visualization_hook'],
...
custom_hooks = [
    dict(
        ...
        num_samples=10,  # Number of deterministic samples to visualize
        score_threshold=0.1,  # Score threshold for filtering predictions
        interval=1,  # Run every N epochs (should match evaluation interval)
        priority='LOW',  # Run after EvalHook (which has NORMAL priority)
    ),
]
