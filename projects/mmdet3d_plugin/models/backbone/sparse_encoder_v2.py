...
## Hiep modify this to give the option return the sparse feature with indices or dense feature map
...
class SparseEncoderV2(nn.Module):
    ...
    def __init__(self,
        ...
        super().__init__()
        assert return_type in ['dense', 'sparse'], f"Invalid return type: {return_type}"
    ...
    def make_encoder_layers(self,
        ...
        for i, blocks in enumerate(self.encoder_channels):
            ...
            for j, out_channels in enumerate(tuple(blocks)):
                ...
                # Resolve stride
                if self.encoder_strides is not None:
                    s_i = self.encoder_strides[i]
                    stride = tuple(s_i)[j] if isinstance(s_i, (list, tuple)) else s_i
                    if isinstance(stride, int):
                        stride = (stride, stride, stride)
                else:
                    # Default behavior: downsample on first block of conv_module stages (except first),
                    # and on last block of non-final basicblock stages; otherwise stride=1
                    if block_type == 'conv_module' and i != 0 and j == 0:
                        stride = (2, 2, 2)
                    elif block_type == 'basicblock' and j == len(blocks) - 1 and i != len(self.encoder_channels) - 1:
                        stride = (2, 2, 2)
                    else:
                        stride = (1, 1, 1)
