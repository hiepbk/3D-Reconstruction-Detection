...
def _write_oriented_bbox(scene_bbox, out_filename):
    ...
    if isinstance(mesh_list, list):
        for mesh in mesh_list:
            mesh.export(out_filename, file_type='obj')
    else:
...
def show_result(points,
    ...
    if show:
        ...
        vis = Visualizer(points, window_name=filename, center_mode=center_mode)
        # pred_bboxes with red color
        if pred_bboxes is not None:
            vis.add_bboxes(bbox3d=pred_bboxes, bbox_color=(1, 0, 0))
        # gt_bboxes with green color
        if gt_bboxes is not None:
            vis.add_bboxes(bbox3d=gt_bboxes, bbox_color=(0, 1, 0))
        show_path = osp.join(result_path,
                             f'{filename}_online.png') if snapshot else None
        vis.show(show_path, vis_time=vis_time)
