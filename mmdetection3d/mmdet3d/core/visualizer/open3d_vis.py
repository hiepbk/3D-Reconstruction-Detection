...
def _draw_bboxes(bbox3d,
    ...
    for i in range(len(bbox3d)):
        ...
        if center_mode == 'lidar_bottom':
            ...
            center[rot_axis] -= dim[
                rot_axis] / 2  # bottom center to gravity center
        elif center_mode == 'lidar_gravity':
            center[rot_axis]  = center[rot_axis] # keep gravity center
...
class Visualizer(object):
    ...
    Args:
        ...
        mode (str):  indicate type of the input points, avaliable mode
            ['xyz', 'xyzrgb']. Default: 'xyz'.
        window_name (str): name of the window. Default: None.
    """
    ...
    def __init__(self,
        ...
        self.window_name = window_name if window_name else 'Open3D'
        self.o3d_visualizer.create_window(window_name=self.window_name)
        mesh_frame = geometry.TriangleMesh.create_coordinate_frame(
            size=1, origin=[0, 0, 0])  # create coordinate frame
    ...
    def show(self, save_path=None, vis_time=None):
        """Visualize the points cloud.

        Args:
            save_path (str): path to save image. Default: None.
            vis_time (float): Time to display the visualization.
                Default: 1.0, None means wait for user to close the window.
        """
        ...
        if vis_time is None:
            # Run in blocking mode - wait for user to close window
            self.o3d_visualizer.run()
        else:
            # Show for specified time
            time.sleep(vis_time)
